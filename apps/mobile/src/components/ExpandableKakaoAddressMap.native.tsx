...
export function ExpandableKakaoAddressMap(props: KakaoAddressMapProps) {
  ...
  return (
    <>
      ...
      <Modal animationType="fade" onRequestClose={() => setExpanded(false)} statusBarTranslucent visible={expanded}>
        <SafeAreaView edges={["top", "bottom"]} style={styles.fullscreen}>
          ...
          {pendingSelection ? (
            <Pressable accessibilityLabel="선택한 위치 확인" accessibilityRole="button" onPress={confirmSelection} style={styles.fullscreenConfirmButton}>
              <Text numberOfLines={1} style={styles.confirmButtonText}>선택한 위치 확인</Text>
            </Pressable>
          ) : null}
        </SafeAreaView>
      </Modal>
