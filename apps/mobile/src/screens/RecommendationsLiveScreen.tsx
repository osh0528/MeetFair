...
export function RecommendationsLiveScreen({ navigation, route }: Props) {
  ...
  const [items, setItems] = useState<MeetingRecommendation[]>([]);
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      setMessage(recommendationError(error));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const voteCounts = useMemo(() => new Map(
    (meeting?.placeCandidates ?? []).map((candidate) => [candidate.id, candidate.votes.length]),
      ...
      {selected ? (
        <View style={styles.footer}>
          <Button
            ...
            disabled={!selected.id || voting || meeting?.status !== "PLANNING" || Boolean(meeting?.confirmedPlace)}
            onPress={() => void vote()}
          />
        </View>
      ) : null}
...
function State({ title, body, retry }: { title: string; body: string; retry?: () => void }) {
  return <Card style={styles.state}><Text style={styles.stateTitle}>{title}</Text><Text style={styles.stateBody}>{body}</Text>{retry ? <Button compact label="다시 시도" onPress={retry} /> : null}</Card>;
}
...
const styles = StyleSheet.create({
  ...
  address: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 4 },
