...
export async function searchNearbyKakaoPlaces(input: {
  ...
  if (!env.KAKAO_REST_API_KEY) {
    throw new AppError(503, "KAKAO_LOCAL_NOT_CONFIGURED", "Kakao REST API ?ㅻ? ?쒕쾭???ㅼ젙??二쇱꽭??");
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new AppError(502, "KAKAO_LOCAL_FAILED", `Kakao ?μ냼 寃?됱뿉 ?ㅽ뙣?덉뒿?덈떎: ${response.status}`);
    }
    ...
    throw new AppError(502, "KAKAO_LOCAL_FAILED", "Kakao ?μ냼 寃?됱뿉 ?ㅽ뙣?덉뒿?덈떎.");
  } finally {
